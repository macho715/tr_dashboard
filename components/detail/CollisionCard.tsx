...
export function CollisionCard({ collision, onClick }: CollisionCardProps) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex w-full items-start gap-2 rounded-lg border px-3 py-2 text-left text-[11px] transition ${severityStyle}`}
      data-testid="collision-card"
    >
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="font-semibold text-foreground">{collision.activity_id}</span>
          <span className="rounded border border-slate-600/60 bg-slate-800/60 px-1.5 py-0.5 text-[10px] text-slate-400">
            {typeLabel}
          </span>
